...
public class MediaPickerActivity extends AppCompatActivity
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    if (getSupportActionBar() != null) {
      getSupportActionBar().setBackgroundDrawable(
          getResources().getDrawable(R.drawable.picker_actionbar_translucent));
      ...
      if (isPhoto) {
        if (mMediaOptions.isCropped() && !mMediaOptions.canSelectMultiPhoto()) {
          ...
          MediaItem mediaItem = new MediaItem(MediaItem.PHOTO,
              ((MediaPickerFragment) activePage).getMediaSelectedList().get(0).getUriOrigin());
  ...
  @Override public MediaImageLoader getImageLoader() {
    return new MediaImageLoaderImpl(getApplicationContext());
  }

  @Override public void onHasNoSelected() {
    mDone.setVisible(false);
    syncActionbar();
  }

  @Override public void onHasSelected(List<MediaItem> mediaSelectedList) {
    showDone();
  }
  ...
  private RecursiveFileObserver.OnFileCreatedListener mOnFileCreatedListener =
      new RecursiveFileObserver.OnFileCreatedListener() {
    ...
    if (takeVideoIntent.resolveActivity(getPackageManager()) != null) {
      ...
      if (max != Integer.MAX_VALUE) {
        ...
        if (mMediaOptions.isShowWarningVideoDuration()) {
          MediaPickerErrorDialog dialog = MediaPickerErrorDialog.newInstance(
              MessageUtils.getWarningMessageVideoDuration(getApplicationContext(), max));
  ...
  private void tryCorrectPhotoFileCaptured() {
    ...
    for (File file : mFilesCreatedWhileCapturePhoto) {
      if (MediaUtils.isImageExtension(MediaUtils.getFileExtension(file))
        ...
        mPhotoFileCapture = file;
        Log.i(TAG, String.format("Try correct photo file: Delete duplicate photos in [%s] [%s]",
  ...
  @Override public void onSuccess(MediaItem mediaItem) {
  ...
  private void hideAllOptionsMenu() {
    if (mPhoto != null) mPhoto.setVisible(false);
    if (mVideo != null) mVideo.setVisible(false);
    if (mMediaSwitcher != null) mMediaSwitcher.setVisible(false);
    if (mDone != null) mDone.setVisible(false);
  }
  ...
  private void returnVideo(Uri videoUri) {
    ...
    switch (code) {
      ...
      case -1:
        ...
        String msg =
            MessageUtils.getInvalidMessageMinVideoDuration(getApplicationContext(), duration);
  ...
  public static ArrayList<MediaItem> getMediaItemSelected(Intent intent) {
    ...
    ArrayList<MediaItem> mediaItemList =
        intent.getParcelableArrayListExtra(MediaPickerActivity.EXTRA_MEDIA_SELECTED);
  ...
  private class FileObserverTask extends AsyncTask<Void, Void, Void> {
    ...
    @Override protected Void doInBackground(Void... params) {
      ...
      if (mFileObserver == null) {
        mFileObserver =
            new RecursiveFileObserver(Environment.getExternalStorageDirectory().getAbsolutePath(),
                FileObserver.CREATE);
