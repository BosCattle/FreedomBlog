...
public class MediaUtils {
  ...
  public static int getLastImageId(Context context) {
    ...
    Cursor cursor = context.getContentResolver()
        .query(Images.Media.EXTERNAL_CONTENT_URI, imageColumns, null, null, imageOrderBy);
  ...
  public static String checkNull(Context context, int lastImageId, File fileCapture) {
    ...
    Cursor cursor =
        contentResolver.query(Images.Media.EXTERNAL_CONTENT_URI, imageColumns, imageWhere,
    ...
    if (cursor.getCount() >= 2) {
      for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
        ...
        if (data.equals(fileCapture.getPath())) {
          int rows =
              contentResolver.delete(Images.Media.EXTERNAL_CONTENT_URI, Images.Media._ID + "=?",
