package org.jiangtao.utils;

/**
 * Created by MrJiang on 2016/1/10.
 */
public final class StaticResources {
  /**
   * 请求码
   */
  public static final int requestCodeLogin = 0x000;
  public static final int requestCodeRegister = 0x111;

  /**
   * 第三方key与sercret
   */
  public static final String MOB_SMS_KEY = "f12b9a7318da";
  public static final String MOB_SMS_SECRET = "448e363cde8a960c56917f704cb9e4f4";
  /**
   * 网址
   */
  public static final String BASE_URL = "http://159.203.129.92:8080/Monkey_war_exploded/";
  //public static final String BASE_URL = "http://192.168.199.227:8080/";
  public static final String BASE_URL_FIR = " http://api.fir.im/apps/";
}
